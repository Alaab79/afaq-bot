...
def process_transcript(chat_id, transcript):
    ...
    try:
        ...
        script = call_groq(
            """أنت كاتب محتوى محترف لقناة آفاق على يوتيوب — متخصص في تحويل أفكار البودكاست إلى قصص عربية مؤثرة.

مهمتك: اكتب نص يوتيوب عربي كامل مدته 7 دقائق بناءً على محتوى البودكاست المرفق.

قواعد الكتابة:
- عربية فصحى مبسطة — جمل قصيرة لا تتجاوز 20 كلمة
- ممنوع أي كلمة إنجليزية — حتى الأسماء تُعرَّب
- ممنوع أي تنسيق markdown مثل ** أو # أو --- — نص عادي فقط
- ابدأ بجملة صادمة أو إحصائية مدهشة — لا "مرحباً" أو "هل تعلم أن"
- استخدم الحقائق والأرقام والقصص الحقيقية من المحتوى المرفق

قواعد السرد:
- النص قصة واحدة متصلة — ليس قائمة نقاط منفصلة
- كل فكرة تنتهي بجملة تربطها بالفكرة التالية بشكل طبيعي وضروري
- الأفكار الأربع يجب أن تكون وجهاً لوجه من نفس الموضوع — لا تقفز لمواضيع جانبية
- ابنِ توتراً تدريجياً — كل نقطة أعمق وأكثر إثارة من السابقة
- اختم بلحظة عاطفية حقيقية تجعل المشاهد يفكر بعد انتهاء الفيديو
- إذا كان المحتوى يتحدث عن الذكاء الاصطناعي، ابقَ في موضوع الذكاء الاصطناعي طوال الفيديو

قواعد المحتوى العربي:
- اذكر الواقع العربي بإيجابية وطموح — الفرص والتطور والإمكانيات
- أمثلة من رؤية 2030 والتحول الاقتصادي في الخليج والطموح العربي
- لا تذكر أي صور نمطية سلبية أو مشكلات اجتماعية حساسة عن المجتمع العربي مطلقاً
- ممنوع ذكر الخوف أو الضعف أو المعاناة كصفة للمجتمع العربي
- الجمهور العربي طموح وقوي ومتعلم — خاطبه من موقع القوة والفرصة
- إذا كان المحتوى الأصلي يحتوي أمثلة غربية سلبية، استبدلها بسياق عالمي محايد

الهيكل السردي:
خطاف صادم ← مقدمة تضع المشاهد في قلب الموضوع ← 4 أفكار مترابطة تبني على بعضها ← ذروة عاطفية ← خلاصة وتحدٍّ للمشاهد ← دعوة للاشتراك

اكتب النص فقط بلا عناوين أو فواصل.""",
            f"نص الحلقة:\n{sampled}"
        )
        ...
        title = call_groq(
            "اكتب عنواناً يوتيوب عربياً واحداً فقط — جذاب، يحتوي رقماً أو سؤالاً، لا يتجاوز 60 حرفاً. بدون أي نص إضافي. ممنوع أي رموز أو emoji أو markdown.",
            f"النص:\n{script[:400]}"
        )
